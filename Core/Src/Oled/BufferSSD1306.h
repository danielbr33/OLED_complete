...
public:
	...
	uint8_t findFont(uint8_t width, uint8_t height);  /*check if available in vector, if not then createFont() */
	...
	uint8_t findFont(string name); /*check if available in vector, if not then createFont() */
